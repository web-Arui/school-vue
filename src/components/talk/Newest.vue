          ...
          <div class="listAvatar">
            <div>
              <img
                ...
                alt
              />
              <span>{{" "+item.user[0].name}}</span>
            
            </div>
            <div class="category">{{item.class[0].category}}</div>
          </div>
          <div class="listItem">
  ...
  methods: {
    ...
    async onLoad() {
      ...
      const res = await this.$http.get(
        `/talks?pagesize=5&pagenumber=${this.pagenumber}`
      );
      const {
        data,
        meta: { status }
    ...
    async initData() {
      // 初始化数据
      const res = await this.$http.get(`/talks?pagesize=5&pagenumber=1`);
      const {
        data,
        meta: { status }
      } = res.data;
